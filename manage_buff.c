 ...
 * escription: If b->buf[n] is not pointing at a legal command
 * character, function will do nothing. Otherwise overwrites word.
 */
 ...
 * Description: Complex calculation to move buffer for string insertion. Since
 * all valid command replacements happen at words,function will delete word at
 * position n, & only move buffer if thr isn't enough whitespace for insertion.
 */
